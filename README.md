...
**NOTE**: Sami Huhtiniemi kindly provided the solution for this demonstration how to store the private key in [ec2.tf](terraform/modules/ec2/ec2.tf) so that the key is stored with UTF-8 encoding and that the file permissions are set so that ssh client won't complain about unprotected key file. Thanks Sami!
